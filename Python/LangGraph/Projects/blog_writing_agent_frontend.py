...
# -----------------------------
from check_blog_writing_agent_backend_copy import app
...
#     Yields ("updates"/"values"/"result", payload).
#     """
#     try:
#         for step in graph_app.stream(inputs, stream_mode="updates"):
#             yield ("updates", step)
...
if run_btn:
    ...
    for kind, payload in try_stream(app, inputs):
        if kind in ("updates", "values"):
            ...
            log("[result] received result state")
...
if out:
    ...
    with tab_preview:
        ...
        result_md = out.get("result") or ""
        if not result_md:
            st.warning("No result markdown found.")
        else:
            render_markdown_with_local_images(result_md)
            ...
            if hasattr(plan_obj, "blog_title"):
                ...
                blog_title = extract_title_from_md(result_md, "blog")
            ...
            bundle = bundle_zip(result_md, md_filename, Path("images"))
            st.download_button(
